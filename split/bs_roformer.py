...
class LinearAttention(Module):
    ...
    def forward(self, x):
        x = self.norm(x)
        qkv = self.to_qkv(x)
        q, k, v = rearrange(qkv, "b n (qkv h d) -> qkv b h d n", qkv=3, h=self._heads)
        q, k = map(l2norm, (q, k))
...
class BSRoformer(Module):
    ...
    def __init__(
        ...
        *,
        ...
        if use_pope:
            raise NotImplementedError("PoPE embeddings are not supported in the desktop app runtime.")
    ...
    def forward(self, raw_audio, target=None, active_stem_ids=None, return_loss_breakdown=False):
        ...
        for index, transformer_block in enumerate(self.layers):
            if len(transformer_block) == 3:
                ...
                x, ft_ps = pack([x], "b * d")
                x = checkpoint(linear_transformer, x, use_reentrant=False) if self.use_torch_checkpoint else linear_transformer(x)
            ...
            x, ps = pack([x], "* t d")
            x = checkpoint(time_transformer, x, use_reentrant=False) if self.use_torch_checkpoint else time_transformer(x)
            (x,) = unpack(x, ps, "* t d")

            x = rearrange(x, "b f t d -> b t f d")
            x, ps = pack([x], "* f d")
            x = checkpoint(freq_transformer, x, use_reentrant=False) if self.use_torch_checkpoint else freq_transformer(x)
            (x,) = unpack(x, ps, "* f d")
        ...
        if self.zero_dc:
            # MPS does not support index_fill on complex tensors, so zero the DC bin
            # through the real-valued view instead.
            stft_repr_real = torch.view_as_real(stft_repr)
            stft_repr_real[:, 0, :, :] = 0.0
            stft_repr = torch.view_as_complex(stft_repr_real)
