...
if [[ -f "${INFO_PLIST}" ]]; then
  ...
  /usr/libexec/PlistBuddy -c "Set :CFBundleShortVersionString 0.5.0" "${INFO_PLIST}" >/dev/null 2>&1 || \
    /usr/libexec/PlistBuddy -c "Add :CFBundleShortVersionString string 0.5.0" "${INFO_PLIST}" >/dev/null
  /usr/libexec/PlistBuddy -c "Set :CFBundleVersion 0.5.0" "${INFO_PLIST}" >/dev/null 2>&1 || \
    /usr/libexec/PlistBuddy -c "Add :CFBundleVersion string 0.5.0" "${INFO_PLIST}" >/dev/null
